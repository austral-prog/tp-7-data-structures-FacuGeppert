...
def find_max_value(diccionario):
    ...
    for i in diccionario:
        if maxim is None or diccionario[i] > contador:
...
def find_biggest_expense(gastos):
    ...
    if gastos == {}:
        ...
        for g in gastos:
            for i in gastos[g]:
                suma =+ i
            suma = suma / len(gastos[g])
            if (maxim == None) or suma > maxim:
                maxim = suma
                respuesta = g
            suma = 0
    return respuesta
